...
PanelHeader::PanelHeader(const std::string& text) : title(text) {
...
void NavigatorPanel::setView(DocumentView* v) {
    view = v;
    if (thumbnail) thumbnail->view = v;
    updateZoomLabel();
}

void NavigatorPanel::updateZoomLabel() {
    if (view && zoomLabel) {
...
void NavigatorPanel::render(Framebuffer& fb) {
...
void NavigatorPanel::setEnabled(bool isEnabled) {
    if (zoomSlider) {
...
void LayerPropsPanel::onActiveLayerChanged(i32 index) {
    rebuildForActiveLayer();
}

void LayerPropsPanel::onLayerChanged(i32 index) {
    if (document && index == document->activeLayerIndex) {
        updateCommonControls();
    }
}

void LayerPropsPanel::updateCommonControls() {
    if (!document) return;
    LayerBase* layer = document->getActiveLayer();
    if (!layer) return;
...
void LayerPropsPanel::updateLockedState() {
    if (!document) return;
    LayerBase* layer = document->getActiveLayer();
...
void LayerPropsPanel::setEnabledRecursive(Widget* widget, bool enabled) {
    if (!widget) return;
...
void LayerPropsPanel::notifyAdjustmentChanged() {
    if (document) {
        document->notifyLayerChanged(document->activeLayerIndex);
        getAppState().needsRedraw = true;
    }
...
void LayerPropsPanel::setEnabled(bool isEnabled) {
...
LayerListItem::LayerListItem(i32 index, Document* doc) : layerIndex(index), document(doc) {
    bgColor = Config::COLOR_PANEL;
...
void LayerListItem::setDisabled(bool d) {
    disabled = d;
...
void LayerListItem::confirmEdit() {
    if (!editing) return;
    if (document && layerIndex >= 0 && layerIndex < static_cast<i32>(document->layers.size())) {
        LayerBase* layer = document->getLayer(layerIndex);
...
void LayerListItem::cancelEdit() {
    endEditing();
}

void LayerListItem::updateFromLayer() {
    if (editing) return;
    if (!document || layerIndex < 0 || layerIndex >= static_cast<i32>(document->layers.size())) return;
    LayerBase* layer = document->getLayer(layerIndex);
...
void LayerListItem::renderSelf(Framebuffer& fb) {
    if (disabled) bgColor = Config::COLOR_BACKGROUND_DISABLED;
    else if (selected) bgColor = Config::GRAY_400;
...
void LayerPanel::closeAdjustmentMenu() {
...
void LayerPanel::setEditMode(LayerListItem* item) {
    editingItem = item;
    updateDisabledState();
}

void LayerPanel::clearEditMode() {
    editingItem = nullptr;
    updateDisabledState();
}
...
void LayerPanel::onLayerAdded(i32 index) {
    if (!editingItem) rebuildLayerList();
}

void LayerPanel::onLayerRemoved(i32 index) {
    if (!editingItem) rebuildLayerList();
}

void LayerPanel::onLayerMoved(i32 from, i32 to) {
    if (!editingItem) rebuildLayerList();
}

void LayerPanel::onLayerChanged(i32 index) {
    /* update thumbnail */
}

void LayerPanel::onActiveLayerChanged(i32 index) {
    if (!editingItem) updateSelection();
}

void LayerPanel::setEnabled(bool isEnabled) {
